...
void compress(FILE*, FILE*, map<string,unsigned int>*);
void decompress(FILE*, FILE*, mytable*);
void init_table(mytable*);
void free_table(mytable*);
void write_table(mytable* , entry*);
void init_map(map<string,unsigned int>*);
void write_map(map<string,unsigned int>* , string, int);
 ...
 *
 ****************************************************************/
void compress(FILE *input, FILE *output, map<string,unsigned int>*mymap)
{
	...
	while(TRUE)
	{
		...
		i++;
		//search pc
		map<string,unsigned int>::const_iterator find_result = mymap->find(p+c);
		//found
...
void init_map(map<string,unsigned int>* mymap)
{
	int i;
	for(i=0;i<=255;i++)
	{
...
void write_map(map<string,unsigned int>* mymap, string entry, int i)
{
	//if not full
	...
	{
        printf("Map full when read %d bytes... Clear the map.\n", i);
		mymap->clear();
		init_map(mymap);
	}
}